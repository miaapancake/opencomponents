...
const SimpleTemplate = ({}: FormProps) => {
    return (
        <Form
            style={{ maxWidth: 500 }}
            onSubmit={(obj) => alert(JSON.stringify(obj, undefined, 4))}
        >
            <h1>Login</h1>
            <Label htmlFor="username">Username</Label>
            <TextInput validation={Joi.string().required()} name="username" />
            <Label htmlFor="password">Password</Label>
            <TextInput validation={Joi.string().required()} name="password" type="password" />
            <Form.Submit>Sign In</Form.Submit>
        </Form>
    );
};

export const SimpleLoginFormExample = SimpleTemplate.bind({});

SimpleLoginFormExample.args = {};

const AdvancedTemplate = ({}: FormProps) => {
    return (
...
export const AdvancedExample = AdvancedTemplate.bind({});
